...
TCM_dat1 <- read_csv("TCM_raw/08292023_Current.csv")
...
TCM_dat1 %>% 
  ...
  geom_smooth() +
  xlab("")+
  xlim(c(as.POSIXct("2023-09-11 00:00:00"), as.POSIXct("2023-09-11 10:50:00")))

#We would not have dove before 9:00 AM on retrieval date, so I am removing data points after that time 
#9 AM row number 


TCM_dat1 <- TCM_dat1[-c(1855:nrow(TCM_dat1)),]
...
```{r}
TCM_temp <- read_csv("TCM_raw/08292023_Temperature.csv")
```
...
#Plot final day
TCM_temp %>% ggplot(aes(x = `ISO 8601 Time`, y = `Temperature (C)`)) +
  geom_line() + xlim(c(as.POSIXct("2023-09-11 00:00:00"), as.POSIXct("2023-09-11 10:30:00")))

#We will delete data after at 9 AM, row number
#Find row number
view(TCM_temp)
TCM_temp <- TCM_temp[-c(619: nrow(TCM_temp)),]
...
```{r}
write_csv(TCM_SUM1, "TCM_enviro/08292023_Avg_Current.csv")
write_csv(TCM_temp, "TCM_enviro/08292023_Temp.csv")
```
