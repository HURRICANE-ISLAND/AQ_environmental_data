...
TCM_dat1 <- read_csv("06092023_Current.csv")
...
TCM_dat1 %>% 
  ...
  geom_smooth() +
  xlab("")+
  xlim(c(as.POSIXct("2023-07-27 00:00:00"), as.POSIXct("2023-07-27 10:50:00")))

#We would not have dove before 9:00 AM on retrieval date, so I am removing data points after that time 
#9 AM is row 6887

TCM_dat1 <- TCM_dat1[-c(6887:nrow(TCM_dat1)),]
...
```{r}
TCM_temp <- read_csv("06092023_Temperature.csv")
```
...
#Plot final day
TCM_temp %>% ggplot(aes(x = `ISO 8601 Time`, y = `Temperature (C)`)) +
  geom_line() + xlim(c(as.POSIXct("2023-07-27 00:00:00"), as.POSIXct("2023-07-27 10:30:00")))

#We will delete data after at 9 AM, row 2296

TCM_temp <- TCM_temp[-c(2296, nrow(TCM_temp)),]
...
```{r}
write_csv(TCM_SUM1, "06092023_Avg_Currnet.csv")
write_csv(TCM_temp, "06092023_Temp.csv")
```
